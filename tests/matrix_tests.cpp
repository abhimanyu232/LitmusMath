...
TEST_F(MatrixTest, ConstructFromMatrix) {
  ...
  // copy int from float : does static cast of each element
  Matrix<int> copy_imatrix3(fMatrix1);
  EXPECT_TRUE(copy_imatrix3.size() == 6);
  EXPECT_TRUE(copy_imatrix3.shape().first == 2);
  EXPECT_TRUE(copy_imatrix3.shape().second == 3);
  for (size_t i = 0; i < copy_imatrix3.size(); ++i) {
	EXPECT_TRUE(copy_imatrix3[i] == static_cast<int>(fMatrix1[i]));
  }
}
...
TEST_F(MatrixTest, MatrixSimpleArithmetic) {
  ...
  Matrix<float_type> temp_sum_matrix_result(
	  std::vector<float_type>{300.1, 600.2, 900.3, 1200.4, 1500.5, 1800.6}, 2,
	  3);
  for (size_t i = 0; i < temp_i_Matrix.size(); ++i) {
