...
class StatisticsTest : public testing::Test {
 protected:
	...
	std::vector<float_type> float_vec_2{
		56.6875, 111.1771, 67.6534,	 88.7396,	 11.1050, 107.3175, 51.2599, 18.6375,
		13.0012, 90.7213,	 105.7054, 3.0538,	 6.1273,	29.9811,	95.6204, 33.6322,
		45.3466, 21.6981,	 3.3258,	 101.1523, 26.9900, 18.0077,	17.7181, 31.3482,
...
TEST_F(StatisticsTest, IntegerDataTest) {
	...
	EXPECT_FLOAT_EQ(int_vector_stats.mean, ivec1_stats_actual[0]);
	EXPECT_FLOAT_EQ(int_vector_stats.median, ivec1_stats_actual[1]);
	EXPECT_FLOAT_EQ(int_vector_stats.variance, ivec1_stats_actual[2]);
	// todo: fix variance calculation, tighten test bounds
	EXPECT_FLOAT_EQ(int_vector_stats.sample_variance, ivec1_stats_actual[3]);
	EXPECT_FLOAT_EQ(int_vector_stats.geometric_mean, ivec1_stats_actual[4]);

	int_vector_stats.ResetSummaryStats();

	int_vector_stats.UpdateSummaryStats<int>(int_vec_2);
	// int_vector_stats.PrintSummaryStats();
	EXPECT_FLOAT_EQ(int_vector_stats.mean, ivec2_stats_actual[0]);
	EXPECT_FLOAT_EQ(int_vector_stats.median, ivec2_stats_actual[1]);
	EXPECT_FLOAT_EQ(int_vector_stats.variance, ivec2_stats_actual[2]);
	// todo: fix variance calculation, tighten test bounds
	EXPECT_FLOAT_EQ(int_vector_stats.sample_variance, ivec2_stats_actual[3]);

	EXPECT_FLOAT_EQ(int_vector_stats.geometric_mean, ivec2_stats_actual[4]);
}
...
TEST_F(StatisticsTest, FloatDataTest) {
	...
	EXPECT_FLOAT_EQ(float_vector_stats.mean, fvec1_stats_actual[0]);
	EXPECT_FLOAT_EQ(float_vector_stats.median, fvec1_stats_actual[1]);

	EXPECT_FLOAT_EQ(float_vector_stats.variance, fvec1_stats_actual[2]);
	// todo: fix variance calculation, tighten test bounds
	EXPECT_FLOAT_EQ(float_vector_stats.sample_variance, fvec1_stats_actual[3]);

	EXPECT_FLOAT_EQ(float_vector_stats.geometric_mean, fvec1_stats_actual[4]);
	...
	EXPECT_FLOAT_EQ(float_vector_stats.mean, fvec2_stats_actual[0]);
	EXPECT_FLOAT_EQ(float_vector_stats.median, fvec2_stats_actual[1]);

	EXPECT_FLOAT_EQ(float_vector_stats.variance, fvec2_stats_actual[2]);
	// todo: fix variance calculation, tighten test bounds
	EXPECT_FLOAT_EQ(float_vector_stats.sample_variance, fvec2_stats_actual[3]);

	EXPECT_FLOAT_EQ(float_vector_stats.geometric_mean, fvec2_stats_actual[4]);
}